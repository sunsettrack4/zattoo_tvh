...
then
	...
	do
		...
		then
			...
			sed -i 's/.*/curl -X GET --cookie "\$session" "https:\/\/\$provider\/zapi\/v2\/cached\/program\/power_details\/$powerid?program_ids=&" | grep ": true, " > \$date\/& 2> \/dev\/null/g' filecheck
